...
\begin{align}
  u_{\text{exec}} = & f\Big(\sum_{x = 1}^{k}x_{k,\, t = 1}, u_{\text{legis}}, u_{\text{jud}}\Big) + g_{2}(\text{reelected}) \times f\Big(\sum_{x = 1}^{k}x_{k,\, t = 2}, u_{\text{legis}}, u_{\text{jud}}\Big) \times \frac{1}{(1 + r)} \label{eq:eq_exec} \\
  u_{\text{legis}} = & f\Big(\sum_{x = 1}^{k}x_{k,\, t = 1}, u_{\text{exec}}, u_{\text{jud}}\Big) + \sum_{t = 2}^{t} \Bigg[ g_{t}(\text{reelected}) \times f\Big(\sum_{x = 1}^{k} x_{k, \, t}, u_{\text{exec}}, u_{\text{jud}}\Big) \times \frac{1}{(1 + r)^{t}} \Bigg] \label{eq:eq_legis} \\
  u_{\text{jud}} = & f\Big(\sum_{x = 1}^{k}x_{k,\, t = 1}, u_{\text{exec}}, u_{\text{legis}}\Big) + f\Big(\sum_{t = 2}^{t} \sum_{x = 1}^{k} x_{k, \, t}, u_{\text{exec}}, u_{\text{legis}}\Big) \times \frac{1}{(1 + r)^{t - 1}} \label{eq:eq_jud}
\end{align} \\

I am interested in the relationship developed between agents of different branches of government. Their utilities are conditional on $k$ observable factors, the other branches utility $u = $ \{exec, legis, jud\}, which computed over two time periods for executive agents, undefined periods for legislative, and in perpetuity for judiciary. Executive and legislative agents only sum future utility if reelection function $g_{t}$ is equal to one each time they get elected (period two for executive and many consecutive periods for legislative). These are essentially the same utility functions under different term-limits. If a legislator is reelected just once, then their utility function is symmetric to that of the executive; if they are reelected multiple times, their utility is symmetric to that of the judiciary.
